             ...
             const polyveck *t1)
{
  ...
  VITIS_LOOP_21_1: for(i = 0; i < 32; ++i)
    pk[i] = rho[i];
  pk += 32;

  VITIS_LOOP_25_2: for(i = 0; i < 4; ++i)
               ...
               const uint8_t pk[(32 + 4*320)])
{
  ...
  VITIS_LOOP_44_1: for(i = 0; i < 32; ++i)
    rho[i] = pk[i];
  pk += 32;

  VITIS_LOOP_48_2: for(i = 0; i < 4; ++i)
             ...
             const polyveck *s2)
{
  ...
  VITIS_LOOP_75_1: for(i = 0; i < 32; ++i)
    sk[i] = rho[i];
  sk += 32;

  VITIS_LOOP_79_2: for(i = 0; i < 32; ++i)
    sk[i] = key[i];
  sk += 32;

  VITIS_LOOP_83_3: for(i = 0; i < 48; ++i)
    sk[i] = tr[i];
  sk += 48;

  VITIS_LOOP_87_4: for(i = 0; i < 4; ++i)
    pqcrystals_dilithium2_ref_polyeta_pack(sk + i*96, &s1->vec[i]);
  sk += 4*96;

  VITIS_LOOP_91_5: for(i = 0; i < 4; ++i)
    pqcrystals_dilithium2_ref_polyeta_pack(sk + i*96, &s2->vec[i]);
  sk += 4*96;

  VITIS_LOOP_95_6: for(i = 0; i < 4; ++i)
               ...
               const uint8_t sk[(2*32 + 48 + 4*96 + 4*96 + 4*416)])
{
  ...
  VITIS_LOOP_122_1: for(i = 0; i < 32; ++i)
    rho[i] = sk[i];
  sk += 32;

  VITIS_LOOP_126_2: for(i = 0; i < 32; ++i)
    key[i] = sk[i];
  sk += 32;

  VITIS_LOOP_130_3: for(i = 0; i < 48; ++i)
    tr[i] = sk[i];
  sk += 48;

  VITIS_LOOP_134_4: for(i=0; i < 4; ++i)
    pqcrystals_dilithium2_ref_polyeta_unpack(&s1->vec[i], sk + i*96);
  sk += 4*96;

  VITIS_LOOP_138_5: for(i=0; i < 4; ++i)
    pqcrystals_dilithium2_ref_polyeta_unpack(&s2->vec[i], sk + i*96);
  sk += 4*96;

  VITIS_LOOP_142_6: for(i=0; i < 4; ++i)
              ...
              const polyveck *h)
{
  ...
  VITIS_LOOP_163_1: for(i=0; i < 32; ++i)
    sig[i] = c[i];
  sig += 32;

  VITIS_LOOP_167_2: for(i = 0; i < 4; ++i)
    pqcrystals_dilithium2_ref_polyz_pack(sig + i*576, &z->vec[i]);
  sig += 4*576;


  VITIS_LOOP_172_3: for(i = 0; i < 80 + 4; ++i)
    sig[i] = 0;

  k = 0;
  VITIS_LOOP_176_4: for(i = 0; i < 4; ++i) {
    VITIS_LOOP_177_5: for(j = 0; j < 256; ++j)
               ...
               const uint8_t sig[(32 + 4*576 + (80 + 4))])
{
  ...
  VITIS_LOOP_205_1: for(i = 0; i < 32; ++i)
    c[i] = sig[i];
  sig += 32;

  VITIS_LOOP_209_2: for(i = 0; i < 4; ++i)
    pqcrystals_dilithium2_ref_polyz_unpack(&z->vec[i], sig + i*576);
  sig += 4*576;


  k = 0;
  VITIS_LOOP_215_3: for(i = 0; i < 4; ++i) {
    VITIS_LOOP_216_4: for(j = 0; j < 256; ++j)
      h->vec[i].coeffs[j] = 0;

    if(sig[80 + i] < k || sig[80 + i] > 80)
      return 1;

    VITIS_LOOP_222_5: for(j = k; j < sig[80 + i]; ++j) {
  ...
  VITIS_LOOP_232_6: for(j = k; j < 80; ++j)
    if(sig[j])
      return 1;
